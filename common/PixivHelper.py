...
def apply_mirror(url, config):
    """Replace pximg host with configured mirror (e.g., i.pixiv.cat) when enabled."""
    if config is None or not getattr(config, "useMirrorHost", False):
        return url
    parsed = urllib.parse.urlparse(url)
    if parsed.netloc.find("pximg.net") == -1:
        return url
    mirror_host = getattr(config, "mirrorHost", "").strip()
    if not mirror_host:
        return url
    # allow host with or without scheme
    if "://" in mirror_host:
        mirror_parsed = urllib.parse.urlparse(mirror_host)
        scheme = mirror_parsed.scheme or parsed.scheme
        netloc = mirror_parsed.netloc
    else:
...
def _mirror_headers(config, referer):
...
def download_image_multithread(url, filename, file_size, overwrite, config, referer):
    """Download file using HTTP range with multiple threads (mirror only)."""
    session = requests.Session()
    headers = _mirror_headers(config, referer)
    part_paths = []
    try:
        head_resp = session.head(url, headers=headers, timeout=config.timeout, allow_redirects=True)
        head_resp.raise_for_status()
        if file_size <= 0:
            try:
                file_size = int(head_resp.headers.get("Content-Length", -1))
            except Exception:
                file_size = -1
        accept_ranges = head_resp.headers.get("Accept-Ranges", "").lower() == "bytes"
        if file_size <= 0 or not accept_ranges:
            raise RuntimeError("Mirror 不支持范围下载或无法获取文件大小")

        chunk_size = max(64 * 1024, int(getattr(config, "mirrorChunkSizeKB", 1024)) * 1024)
        total_chunks = int(math.ceil(file_size / chunk_size))
        workers = max(1, int(getattr(config, "mirrorThread", 4)))
        ranges = []
        start = 0
        idx = 0
        ...
        print_and_log('info', f"Using mirror {urllib.parse.urlparse(url).netloc} with {min(workers, len(ranges))} 线程, 每片 {size_in_str(chunk_size)}")
        makeSubdirs(filename)
        part_paths = [None] * len(ranges)

        def fetch(part):
            p_idx, p_start, p_end = part
            part_path = f"{filename}.part{p_idx}"
            resp = session.get(
                url,
                headers={**headers, "Range": f"bytes={p_start}-{p_end}"},
                stream=True,
            ...
            with open(part_path, "wb") as f_out:
                for chunk in resp.iter_content(1024 * 64):
                    if chunk:
                        f_out.write(chunk)
            part_paths[p_idx] = part_path

        with ThreadPoolExecutor(max_workers=min(workers, len(ranges))) as executor:
            futures = [executor.submit(fetch, r) for r in ranges]
            for f in futures:
                f.result()

        temp_name = filename + ".pixiv"
        with open(temp_name, "wb") as f_out:
            for part_path in part_paths:
                with open(part_path, "rb") as src:
                    shutil.copyfileobj(src, f_out, 1024 * 1024)
        for part_path in part_paths:
            try:
                os.remove(part_path)
            except OSError:
