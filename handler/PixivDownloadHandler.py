...
def download_image(caller,
    ...
    target_url = PixivHelper.apply_mirror(url, config)
    ...
    while retry_count <= max_retry:
        ...
        try:
            try:
                ...
                if is_exists:
                    remote_file_size = get_remote_filesize(target_url, referer, config, notifier)
                else:
                    remote_file_size = -1
                ...
                # actual download
                notifier(type="DOWNLOAD", message=f"Start downloading {target_url} to {filename_save}")
                (downloadedSize, filename_save) = perform_download(target_url, remote_file_size, filename_save, overwrite, config, referer)
                ...
                if remote_file_size > 0 and downloadedSize != remote_file_size:
                    PixivHelper.print_and_log('error', f"Incomplete Download for {target_url} => {filename_save}")
            ...
            PixivHelper.print_and_log('error', f'Error at download_image(): {sys.exc_info()} at {target_url} ({caller.ERROR_CODE})')
...
def perform_download(url, file_size, filename, overwrite, config, referer=None, notifier=None):
    ...
    target_url = PixivHelper.apply_mirror(url, config)
    if config.useMirrorHost and config.mirrorMultithread:
        try:
            return PixivHelper.download_image_multithread(target_url, filename, file_size, overwrite, config, referer)
        except Exception as ex:
            PixivHelper.print_and_log('warn', f"Mirror多线程下载失败，回退单线程: {ex}")
    # actual download
    # PixivHelper.print_and_log(None, '\rStart downloading...', newline=False)
    # fetch filesize
    req = PixivHelper.create_custom_request(target_url, config, referer)
    ...
    (downloadedSize, filename) = PixivHelper.download_image(target_url, filename, res, file_size, overwrite)
    res.close()
    gc.collect()
